...
with app.app_context():
    ...
    if df_composition.empty:
        error_message = "No data available to display the graph."
    ...
    dash_price_review.layout = html.Div([
        dcc.DatePickerRange(
            ...
            start_date=df_price_review['date_extracted'].min() if not df_price_review.empty else '2020-01-01',
            end_date=df_price_review['date_extracted'].max() if not df_price_review.empty else '2024-12-31',
    ...
    def update_scatter_plot(start_date, end_date):
        ...
        # Create the scatter plot (assuming 'price' and 'review' columns exist in the data)
        if not df_filtered.empty:
            fig = px.scatter(df_filtered, x='price', y='review', title="Price vs Review Score",
                            labels={'price': 'Price', 'review': 'Review Score'})
        else:
            fig = px.scatter(title="No data available for the selected date range")
